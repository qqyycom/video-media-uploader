...
export function useDirectUpload({
  ...
  const uploadToPlatform = useCallback(
    async (platform: 'youtube' | 'tiktok', videoFile: VideoFile, metadata: UploadMetadata) => {
      const uploadId = addUpload(platform, videoFile, metadata)
      const accessToken = platform === 'youtube' ? youtubeAccessToken : tiktokAccessToken
        ...
        if (platform === 'youtube') {
          result = await uploadToYouTube({
            ...
            onChunkUploaded: (uploadedBytes) => {
              updateUpload(uploadId, {
                message: `Uploaded ${(uploadedBytes / 1024 / 1024).toFixed(1)} MB`,
              })
            },
          })
        } else {
